 ...
 *
 * @author Nathan Funk
 * $Id: JEP.java,v 1.10 2014/08/20 08:15:00 matog Exp $
 */
...
public class JEP {
	...
	protected Vector errorList;		// NOSONAR kvoli spatnej kompatibilite Vector
	...
	public JEP() {
		...
		initSymTab();
		initFunTab();
		errorList = new Vector();	// NOSONAR kvoli spatnej kompatibilite Exception
	...
	public JEP(boolean traverse_in,
		...
		initSymTab();
		initFunTab();
		errorList = new Vector();	// NOSONAR kvoli spatnej kompatibilite Vector
		ev = new EvaluatorVisitor();
		parser = new Parser(new StringReader(""));

		//Ensure errors are reported for the initial expression
		//e.g. No expression entered
		parseExpression("");	// NOSONAR constructor calls overridable method aj ked je final	
	}
	 ...
	 */
	public final void initSymTab() {	// volana z konstruktoru
		//Init SymbolTable
	 ...
	 */
	public final void initFunTab() {	// volana z konstruktoru
		//Init FunctionTable
	 ...
	 */
	@SuppressWarnings("unchecked")
	public final void parseExpression(String expression_in) {	// volana z konstruktoru
		...
		catch (Throwable e)	//NOSONAR kvoli spatnej kompatibilite chytame Throwable 
		{
			...
			if (e instanceof ParseException) {
				...
				if (debug) {
					System.out.println(e.getMessage());		// NOSONAR len debug - vypnute
					e.printStackTrace();					// NOSONAR len debug - vypnute
				}
	...
	public Node parse(String expression) throws ParseException
	{
		...
		return parser.parseStream(sr, this);
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public Object evaluate(Node node) throws Exception	// NOSONAR kvoli spatnej kompatibilite Exception
	{
		return ev.getValue(node, new Vector(), this.symTab);	// NOSONAR kvoli spatnej kompatibilite Vector
	}
	...
	public Object getValueAsObject() {
		...
		if (topNode != null && !hasError()) {
			...
			try {
				...
				if (debug) {
					System.out.println(e);	// NOSONAR len debug - vypnute
				}
	...
	public String getErrorInfo() {
		if (hasError()) {
			...
			for (int i=0; i<errorList.size(); i++) {
				str.append(errorList.elementAt(i)).append("\n");
			}
